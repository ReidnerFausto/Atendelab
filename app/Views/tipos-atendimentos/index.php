<?php
$tituloPagina = 'Tipos de atendimentos';
require __DIR__ . '/../layout/header.php';
?>

<div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-4">
    <div>
        <h1 class="h3 mb-1">Tipos de atendimentos</h1>
...
<script>
    ...
    async function carregarTipos() {
        try {
            const resposta = await AtendeLabApi.get('tiposatendimentos', 'listarTipoAtendimento');
            ...
            tabelaTipos.innerHTML = tipos.map((tipo) => {
                ...
                <tr>
                    ...
                    <td class="text-end">
                        <button type="button" class="btn btn-sm btn-outline-primary" onclick="editarTipo(${Number(tipo.id)})">Editar</button>
                        <button type="button" class="btn btn-sm btn-outline-danger" onclick="inativarTipo(${Number(tipo.id)})">Inativar</button>
                    </td>
                </tr>
                `;
            }).join('');
    ...
    async function editarTipo(id) {
        try {
            const resposta = await AtendeLabApi.get('tiposatendimentos', 'buscarAtendimento', { id });
    ...
    formTipo.addEventListener('submit', async (event) => {
        ...
        try {
            await AtendeLabApi.post('tiposatendimentos', acao, new FormData(formTipo));
    ...
    async function inativarTipo(id) {
        ...
        try {
            await AtendeLabApi.post('tiposatendimentos', 'inativarTipoAtendimento', { id });
